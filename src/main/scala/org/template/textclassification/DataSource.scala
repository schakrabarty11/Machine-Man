...
class DataSource (
  ...
  private def readEventData(sc: SparkContext) : RDD[Observation] = {
    ...
    PEventStore.find(
      ...
      Observation(
        if (label == "machine") 1.0 else 0.0,
        e.properties.get[String]("folder"),
        e.properties.get[String]("ret"),
        e.properties.get[String]("subject"),
        e.properties.get[String]("from"),
        label
      )
    }).cache
  }
  ...
  Seq[(TrainingData, EmptyEvaluationInfo, RDD[(Query, ActualResult)])] = {
    ...
    (0 until dsp.evalK.get).map { k =>
      ...
      val test = data.filter(_._2 % dsp.evalK.get == k)
        .map(_._1)
        .map(e => (new Query(e.folder, e.ret, e.subject, e.from), new ActualResult(e.category)))
